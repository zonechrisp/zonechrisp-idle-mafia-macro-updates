...
**Beta: v17.2.0 — Smooth Navigation & Macro Library**

v17.2 adds stable scrolling, a compact slot editor, persistent slot names, reusable macros with JSON import/export and a personal sidebar photo. Install from **Updates → Beta → Check Now → v17.2.0**.
...
- `packages/<version>/` contains modern update packages (ZIP or legacy Base64 chunks).
...
Modern update packages are SHA-256 verified before installation. Older chunked packages are concatenated and Base64-decoded first. Modern versions remain side-by-side while current settings, macros, uploaded target images, Popup Recovery data and sequence data stay in shared app data.
